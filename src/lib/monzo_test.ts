...
const ratioEquals = (actual: number, expected: number): boolean =>
  Math.abs(actual / expected - 1) < 1e-12;
...
Deno.test("(Monzo, mul) は可換群: 結合律・交換律", () => {
  fc.assert(
    fc.property(
      ...
      arbMonzo,
      arbMonzo,
      (a, b, c) => equals(mul(mul(a, b), c), mul(a, mul(b, c))) && equals(mul(a, b), mul(b, a)),
    ),
  );
});
...
Deno.test("(Monzo, mul) は可換群: 単位元と逆元", () => {
  fc.assert(
    fc.property(arbMonzo, (a) => equals(mul(a, UNIT), a) && equals(mul(a, inverse(a)), UNIT)),
  );
});

Deno.test("mul の結果は正規形 (指数 0 のエントリを持たない)", () => {
  fc.assert(
    fc.property(arbMonzo, arbMonzo, (a, b) => Object.values(mul(a, b)).every((e) => e !== 0)),
  );
});
...
Deno.test("equals は正規形での構造的等価と一致する", () => {
  ...
  fc.assert(
    fc.property(
      arbMonzo,
      arbMonzo,
      (a, b) => equals(a, b) === (JSON.stringify(a) === JSON.stringify(b)),
    ),
  );
});
...
Deno.test("cellMonzo(x, y, p) は 3^x * p^y を表す", () => {
  fc.assert(
    fc.property(
      ...
      fc.integer({ min: -6, max: 6 }),
      arbLatticePrime,
      (x, y, p) => ratioEquals(ratioValue(cellMonzo(x, y, p)), 3 ** x * p ** y),
    ),
  );
});
